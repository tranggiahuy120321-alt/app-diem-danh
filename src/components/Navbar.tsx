...
import { ClipboardCheck, UserPlus, BarChart3, Sparkles } from 'lucide-react';
import { Logo } from './Logo';

interface NavbarProps {
  activeTab: 'attendance' | 'addStudent' | 'reports';
  setActiveTab: (tab: 'attendance' | 'addStudent' | 'reports') => void;
...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab, totalStudentsCount }) => {
  return (
    <header className="sticky top-0 z-40 bg-white border-b-4 border-yellow-400 p-3 sm:px-8 shadow-sm">
      <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
        ...
        <nav className="hidden sm:flex items-center p-1.5 bg-slate-100 rounded-full border border-slate-200/80">
          <button
            ...
            className={`flex items-center px-5 py-2 text-sm font-black rounded-full transition-all cursor-pointer ${
        ...
        <div className="flex items-center">
          {typeof totalStudentsCount === 'number' && (
            <div className="text-xs font-black px-3.5 py-2 bg-emerald-50 text-emerald-800 rounded-full border-2 border-emerald-200/80 flex items-center shadow-xs">
      ...
      <div className="flex sm:hidden mt-3 p-1 bg-slate-100 rounded-full border border-slate-200 justify-around">
        <button
          onClick={() => setActiveTab('attendance')}
          className={`flex-1 py-2 text-xs font-black rounded-full flex items-center justify-center space-x-1 transition-all ${
            activeTab === 'attendance'
